...
def accumulate_batch_update_stats(
        *,
        X: DataArray2D,
        b: SourceArray2D,
        u: SourceArray3D,
        ...
        n_weights: int,
        doing_newton: bool,
        out_dgm_numer: ComponentsVector,
        out_dc_numer: ComponentsVector,
        out_dc_denom: ComponentsVector,
        out_dalpha_numer: ParamsArray,
        out_dalpha_denom: ParamsArray,
        out_dmu_numer: ParamsArray,
        out_dmu_denom: ParamsArray,
        out_dbeta_numer: ParamsArray,
        out_dbeta_denom: ParamsArray,
        out_drho_numer: ParamsArray,
        out_drho_denom: ParamsArray,
        out_dsigma2_numer: ComponentsVector | None = None,
        out_dsigma2_denom: ComponentsVector | None = None,
        out_dkappa_numer: ParamsArray | None = None,
        out_dkappa_denom: ParamsArray | None = None,
        out_dlambda_numer: ParamsArray | None = None,
        out_dlambda_denom: ParamsArray | None = None,
        out_dbaralpha_numer: ParamsArray | None = None,
        out_dbaralpha_denom: ParamsArray | None = None,
) -> None:
    """Accumulate all per-batch M-step sufficient statistics.

    This is the single-model fast path used by the fit loop.  It mirrors the
    smaller ``accumulate_*`` helpers, but keeps shared masks and reductions local
    to one pass through the batch tensors.
    """
    assert u.shape == ufp.shape == fp.shape == y.shape
    assert rho.shape == sbeta.shape == usum.shape
    assert b.ndim == 2 and X.ndim == 2
    assert vsum.numel() == 1

    out_dgm_numer[0] += vsum

    out_dc_numer += X[:, :n_weights].sum(dim=0)
    out_dc_denom += vsum

    out_dalpha_numer += usum
    out_dalpha_denom += vsum

    ufp_sum = ufp.sum(dim=0)
    out_dmu_numer += ufp_sum

    out_dbeta_numer += usum
    out_drho_denom += usum

    mask_le2 = rho <= 2.0
    mask_gt2 = rho > 2.0
    all_le2 = bool(torch.all(mask_le2).item())
    any_le2 = all_le2 or bool(torch.any(mask_le2).item())
    any_gt2 = bool(torch.any(mask_gt2).item())
    ...
    if any_le2:
        ...
        if torch.any(~torch.isfinite(mu_denom_sum)):
            ...
            y_safe = signs * torch.clamp(torch.abs(y), min=eps)
        ...
        mu_denom_sum = sbeta * mu_denom_sum
        if all_le2:
            out_dmu_denom += mu_denom_sum
        else:
            out_dmu_denom += torch.where(
                mask_le2,
                mu_denom_sum,
                torch.zeros_like(mu_denom_sum),
            )

        beta_denom = torch.sum(ufp * y, dim=0)
        if all_le2:
            out_dbeta_denom += beta_denom
        else:
            out_dbeta_denom += torch.where(
                mask_le2,
                beta_denom,
                torch.zeros_like(beta_denom),
            )

    ufp_fp_sum = None
    if any_gt2:
        ufp_fp_sum = torch.sum(ufp * fp, dim=0)
        if torch.any(~torch.isfinite(ufp_fp_sum)):
            raise RuntimeError("Non-finite generalized-Gaussian mu denominator.")
        mu_denom_hi = sbeta * ufp_fp_sum
        out_dmu_denom += torch.where(
            mask_gt2,
            mu_denom_hi,
            torch.zeros_like(mu_denom_hi),
        )

        tmpy_hi = torch.pow(torch.abs(y), rho[None, :, :])
        beta_denom_hi = torch.sum(u * tmpy_hi, dim=0)
        out_dbeta_denom += torch.where(
            mask_gt2,
            beta_denom_hi,
    ...
    out_drho_numer += tmpy.sum(dim=0)

    if not doing_newton:
        return

    assert out_dsigma2_numer is not None
    assert out_dsigma2_denom is not None
    assert out_dkappa_numer is not None
    assert out_dkappa_denom is not None
    assert out_dlambda_numer is not None
    assert out_dlambda_denom is not None
    assert out_dbaralpha_numer is not None
    assert out_dbaralpha_denom is not None

    out_dsigma2_numer += (b * b).sum(dim=0)
    out_dsigma2_denom += vsum

    if ufp_fp_sum is None:
        ufp_fp_sum = torch.sum(ufp * fp, dim=0)
    out_dkappa_numer += ufp_fp_sum * (sbeta**2)
    out_dkappa_denom += usum

    tmp = fp * y
    tmp -= 1.0
    tmp **= 2
    out_dlambda_numer += (u * tmp).sum(dim=0)
    out_dlambda_denom += usum

    out_dbaralpha_numer[:, :] += usum
    out_dbaralpha_denom[:, :] += vsum
